...
export const ProcessPanel = observer(
  ({ processes, cycles, playerColors }: Props) => {
    ...
    const details = Array(Math.min(len, MAX_PROCESS_DISPLAYED))
      ...
      .map((_, i) => {
        ...
        const playerColor = playerColors[process.player_id];
        ...
        return (
          <details key={i} className="pad-left">
            ...
            <Info title="State">
              {state ? `${state.op()} (${state.exec_at - cycles + 1})` : "Idle"}
            </Info>
            {registers}
          </details>
        );
